...
} from "@utils/animation/objectUtils";
...
const useAnimation = (
  ...
  const defaultProps = useMemo(
    () => getDefaultAnimationProps(elementRef),
    [elementRef]
  );
  ...
  // reset animation when actions impact to pages animation
  useEffect(() => {
    if (
  ...
  const playAnimationIn = properties => {
    ...
    return new Promise(resolve => {
      api.start({
        ...getConfigInRaw(properties.animationId, defaultProps, properties),
  ...
  const playAnimationOut = properties => {
    ...
    return new Promise(resolve => {
      api.start({
        ...getConfigOutRaw(properties.animationId, defaultProps, properties),
        delay: properties.delay || 0,
        reset: true,
        onStart: () => {
          if (playingRef.current) {
